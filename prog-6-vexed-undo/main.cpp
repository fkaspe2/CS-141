...
{
			// ...
      Board board;        // stores the board at each linked
      int moveNumber;     // stores the moveNumber at each linked
      int score;          // stores the score at each linked
      int index;          // stores the index at each linked
      Node *pNext;        // points to the next node in the list
...
void displayList( Node *pHead)
{
    ...
    while (pTemp != NULL ) {
      cout << pTemp->index << ":" << pTemp->moveNumber;
      // does not display arrow once at tail
      if (pTemp->pNext != NULL) {
          cout << "->";
      }
      pTemp= pTemp->pNext;
...
//    the score, which should be restored from the list
//    the currentBoardIndex, which should be restored from the list
void addNodeToList( Node* & pHead, int theMoveNumber,  Board thisBoard, int thisScore, int thisIndex) 
{
    // Create a new node and store current values into it
    // ...
    Node* pCurrent = new Node;
    pCurrent->moveNumber = theMoveNumber;
    pCurrent->board = thisBoard;
    pCurrent->score = thisScore;
    pCurrent->index = thisIndex;
...
int main()
{
    ...
    while( true) {
        ...
        addNodeToList ( pHead, moveNumber, theBoard, score, currentBoardIndex);
        ...
        while( ! theBoard.boardIsClear() ) {
            ...
            if( moveAction == 'U') {
                ...
                if (pHead != NULL && pHead->pNext != NULL) {
                  ...
                  // REMOVES NODE AND RESETS DATA MEMBERS
                  deleteNodeFromList( pHead );
                  theBoard = pHead->board;
                  currentBoardIndex = pHead->index;
                  score = pHead->score;
        ...
        if( moveAction == 'X') {
            ...
            // User input was 'R' to reset the level
                                                    // theBoard = allTheBoards.getBoard( currentBoardIndex);      DELETE AT END????
