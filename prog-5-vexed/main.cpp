 ...
 * - Haha to your comment "Goodnight sweet prince"
 * - Looks good
 * */
//------------------------------------
//  Prog5Vexed.cpp
...
// scans the array to check if it is empty or not
bool checkIfEmpty (Square squaresArray[]) {
    for (int i = 1; i < BoardRows*BoardColumns; i++) 
    {
        sf::Color sourceColor = squaresArray[ i].getColor();
...
// searches for floating pieces after a move is made
bool isThereAFloater (Square squaresArray[]) {
    int sourceIndex;
    int destinationIndex;
    sf::Color sourceColor;
    sf::Color destinationColor;
    
    for (int i = 80; i > 0; i--) 
    {
        sourceIndex = i;
        destinationIndex = i+10;
        // checks to make sure destinationIndex is within the bounds
        if (destinationIndex <= 80) 
        {
            sourceColor = squaresArray[ sourceIndex].getColor();
            destinationColor = squaresArray[ destinationIndex].getColor();
            if ( sourceColor != BlankColor && sourceColor != WallColor
               && destinationColor == BlankColor ) {
                return true;
            }
        }
    }
...
// drops any piece that has a blank space below it to the bottom of the board
void scanAndFall ( Square squaresArray[]) {
    int sourceIndex;
    int destinationIndex;
    sf::Color sourceColor;
    sf::Color destinationColor;
    ...
    for (int i = 80; i > 0; i--) 
    {
        ...
        if (destinationIndex <= 80) 
        {
            sourceColor = squaresArray[ sourceIndex].getColor();
            destinationColor = squaresArray[ destinationIndex].getColor();
            while ( sourceColor != BlankColor && sourceColor != WallColor
                   && destinationColor == BlankColor ) 
            {            
                swapColors( squaresArray[sourceIndex], squaresArray[destinationIndex]);
                sourceIndex += 10;
                destinationIndex += 10;
                sourceColor = squaresArray[ sourceIndex].getColor();
                destinationColor = squaresArray[ destinationIndex].getColor();
...
// recursively checks to see if a board piece has matching adjacent pieces
// marks them using isCaptured = true
void mark ( int position, Square squaresArray[] ) {
    sf::Color sourceColor = squaresArray[position ].getColor();
    
    //check left
    if (sourceColor == squaresArray[position -1 ].getColor() && squaresArray[position -1 ].getIsCaptured() != true) 
    {
        squaresArray[position ].setIsCaptured(true );
        squaresArray[position -1 ].setIsCaptured(true ) ;
        mark(position-1, squaresArray);
    }
    //check right
    if (sourceColor == squaresArray[position +1 ].getColor() && squaresArray[position +1 ].getIsCaptured() != true) 
    {
    ...
    //check up
    if (sourceColor == squaresArray[position -10 ].getColor() && squaresArray[position -10 ].getIsCaptured() != true) 
    {
        squaresArray[position ].setIsCaptured(true );
        squaresArray[position -10 ].setIsCaptured(true ) ;
        mark(position-10, squaresArray);
    }
    //check down
    if (sourceColor == squaresArray[position +10 ].getColor() && squaresArray[position +10 ].getIsCaptured() != true) 
    {
...
// changes pieces that have been marked to the blank color
void blankMatches(Square squaresArray[]) {
    int position = 1;
    while (position < 81) 
    {
        if( squaresArray[position].getIsCaptured() == true) {
            squaresArray[position].setColor(250, 250, 250);
            squaresArray[position].setIsCaptured(false);
        }
        position ++;
    }
}
...
// scans boards to delete pieces
void scanAndDelete ( Square squaresArray[]) {
    sf::Color sourceColor;
    
    int position = 1;
    while (position < 80) {
        sourceColor = squaresArray[ position].getColor();
        if (sourceColor != BlankColor && sourceColor != WallColor) {
            mark( position, squaresArray);
        }
        
        blankMatches( squaresArray);
        position++;
    }
}


// calls necessary functions for game mechanics
void takeAction (Square squaresArray[]) {
    scanAndFall (squaresArray);
    scanAndDelete (squaresArray);
    // checks to see if board has been fully set after move
    while (isThereAFloater(squaresArray) == true) {
        scanAndFall (squaresArray);
        scanAndDelete (squaresArray);
    }
}
...
{	 
	...
	while (window.isOpen())
	{
		...
		swapColors( squaresArray[squareIndex], squaresArray[destinationIndex]);
        takeAction( squaresArray);
        
        int currentPar = allTheBoards.getParValue(currentBoardIndex);
        // If the board is empty at this point, we check if the board is empty or not
        // If the user cleared the level before reaching the par value, go to next level and increase user score
        if (checkIfEmpty(squaresArray) == true && moveNumber <= currentPar ) {
            score += currentPar;
            currentBoardIndex++;
            std::cout << std::endl << "Congratulations!  On to level " << currentBoardIndex << std::endl;
        ...
        // If the user cleared the level but exceeded par, reset the board
        if (checkIfEmpty(squaresArray) == true && moveNumber > currentPar ) { 
            std::cout << std::endl << "Sorry, you took " << moveNumber
                 << " moves and you must finish within "
                 << currentPar << " moves before moving on. " << std::endl;
            // resets level
            moveNumber = 1;
            allTheBoards.setBoardValues( currentBoardIndex, boardValues);
            initializeDisplayWindow( window, backgroundRectangle, boardValues, squaresArray, 
